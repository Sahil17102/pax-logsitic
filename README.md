...
Client warehouse creation is admin-only through `POST /api/admin/delhivery/warehouses`. Pax validates and forwards only Delhivery's documented JSON fields to `/api/backend/clientwarehouse/create/`, persists successful registrations, and rejects exact duplicate names locally because warehouse names are case-sensitive provider identifiers. Full POC and address details remain admin-only; customer bootstrap exposes only registered names and the default flag so a warehouse can be selected safely when manifesting a shipment or scheduling collection. `GET /api/admin/delhivery/warehouses` returns the persisted admin view. The adapter reserves nine requests per one-minute process window below Delhivery's 10-request/minute/IP limit and uses a five-second timeout above the documented production P99.
