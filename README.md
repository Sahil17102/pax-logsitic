...
`POST /api/admin/auth/login` accepts `{ "username": "...", "password": "..." }` and returns `{ "token": "...", "admin": { ... } }`. `GET /api/admin/dashboard` returns `{ "data": { "shipments": [], "pickupRequests": [], "customers": [], "activities": [], "configuration": {} } }`.
