...
An accepted asynchronous action is stored as an append-only Pending record with its UPL ID, and duplicate Pending actions for the same waybill/action are blocked. NDR eligibility metadata and action history remain available only to authenticated owners and admins; public tracking removes NSL, attempt-count, OTP-cancellation and UPL details. Delhivery publishes no rate limit for this endpoint, so Pax uses a configurable conservative 300-request/five-minute local guard and a 130-second timeout to cover the documented 126.38-second P99. The referenced GET NDR Status request/response contract was not supplied, so UPL polling is deliberately not implemented and accepted actions remain Pending until that official contract is added.
