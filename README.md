...
GET   /api/client/pickup-requests
POST  /api/client/pickup-requests
GET   /api/admin/pickup-requests
POST  /api/admin/pickup-requests
PATCH /api/admin/pickup-requests/:id/status
POST  /api/admin/delhivery/warehouses
GET   /api/admin/delhivery/warehouses
PATCH /api/admin/delhivery/warehouses/:id
...
`POST /api/admin/auth/login` accepts `{ "username": "...", "password": "..." }` and returns `{ "token": "...", "admin": { ... } }`. `GET /api/admin/dashboard` returns `{ "data": { "shipments": [], "pickupRequests": [], "warehouses": [], "customers": [], "activities": [], "configuration": {} } }`.
...
DELHIVERY_RVP_QC_QUESTION_IDS=<comma-separated client question IDs mapped by Delhivery>
...
DELHIVERY_DOCUMENT_PATH=/api/rest/fetch/pkg/document/
DELHIVERY_DOCUMENT_RATE_LIMIT_REQUESTS=300
DELHIVERY_DOCUMENT_TIMEOUT_MS=30000
DELHIVERY_NDR_PATH=/api/p/update
DELHIVERY_NDR_RATE_LIMIT_REQUESTS=300
DELHIVERY_NDR_TIMEOUT_MS=130000
DELHIVERY_NDR_STATUS_PATH_TEMPLATE=/api/cmu/get_bulk_upl/{uplId}
DELHIVERY_NDR_STATUS_RATE_LIMIT_REQUESTS=300
DELHIVERY_NDR_STATUS_TIMEOUT_MS=95000
DELHIVERY_PICKUP_PATH=/fm/request/new/
DELHIVERY_PICKUP_RATE_LIMIT_REQUESTS=3600
DELHIVERY_PICKUP_TIMEOUT_MS=5000
DELHIVERY_WAREHOUSE_PATH=/api/backend/clientwarehouse/create/
DELHIVERY_WAREHOUSE_RATE_LIMIT_REQUESTS=9
DELHIVERY_WAREHOUSE_TIMEOUT_MS=5000
DELHIVERY_WAREHOUSE_EDIT_PATH=/api/backend/clientwarehouse/edit/
DELHIVERY_WAREHOUSE_EDIT_TIMEOUT_MS=65000
...
Order documents are fetched through authenticated shipment-scoped `GET /api/client/shipments/:id/document` and `GET /api/admin/shipments/:id/document` routes. Pax verifies shipment ownership and SPS/MPS waybill membership, then calls `/api/rest/fetch/pkg/document/` with only the documented `doc_type` and `waybill` query parameters. Supported types are `SIGNATURE_URL`, `RVP_QC_IMAGE`, `EPOD` and `SELLER_RETURN_IMAGE`. Provider payloads are normalized to a deduplicated list of HTTPS download links; insecure or missing links, unknown types and unrelated waybills are rejected. The customer document tool opens each returned file in a new isolated browser tab. Because the supplied contract does not publish a provider rate limit or latency target, Pax applies a configurable conservative 300-request/five-minute process guard and 30-second timeout without claiming those values as Delhivery limits.

Client warehouse management is admin-only. `POST /api/admin/delhivery/warehouses` validates and forwards only Delhivery's documented creation fields to `/api/backend/clientwarehouse/create/`, persists successful registrations, and rejects exact duplicate names locally because warehouse names are case-sensitive provider identifiers. `PATCH /api/admin/delhivery/warehouses/:id` derives that immutable name from the persisted record and sends only `name`, mandatory `pin`, and supplied `address`/`phone` fields to `/api/backend/clientwarehouse/edit/`; unsupported rename attempts are rejected before the provider call and local details change only after provider acceptance. Full POC and address details remain admin-only; customer bootstrap exposes only registered names and the default flag so a warehouse can be selected safely when manifesting a shipment or scheduling collection. `GET /api/admin/delhivery/warehouses` returns the persisted admin view. Creation and update share a nine-request-per-minute process window below Delhivery's 10-request/minute/IP limit. Creation uses a five-second timeout, while update allows 65 seconds for the documented long-tail latency.

Pickup creation uses authenticated `POST /api/*/pickup-requests` routes and the exact `/fm/request/new/` JSON contract. The caller supplies `pickup_date`, `pickup_time` and `expected_package_count`, and may select one of the exact registered warehouse names; omitting it uses `DELHIVERY_PICKUP_LOCATION`. Arbitrary warehouse names are rejected before Delhivery is called. Dates must be today through the next seven days, times use 24-hour `HH:mm:ss`, and at least one persisted, manifested forward shipment must be ready at that warehouse. Reverse Pickup and REPL orders are excluded because their collections are provider-scheduled. One accepted request is persisted at warehouse/date level, marks all eligible forward packages as pickup scheduled, appears in both customer and admin pickup screens, and blocks another open request for that warehouse/date. After the FE collects the packages and closes the request on their device, an admin can confirm `Completed` locally through `PATCH /api/admin/pickup-requests/:id/status`; only then can Pax attempt another request for that warehouse/date, which Delhivery may still reject if its own request remains open. `GET /api/*/pickup-requests` returns the persisted history with customer ownership filtering. The adapter uses a dedicated 3,600-request process window below Delhivery's 4,000-request/5-minute/IP limit and a five-second timeout above the documented production P99.
...
RVP QC 3.0 extends the same manifestation route for reverse `Pickup` shipments. Customers can enable doorstep QC in the reverse-shipment form and provide one or two items, each with one to six questions. Pax sends hardcoded `qc_type: "param"` plus normalized `custom_qc` data, including item descriptions/images, return reason, quantity, brand/category, mapped `questions_id`, options, correct-value arrays, required-result behavior, `varchar`/`multi` type and optional question images. More than two items or six questions is rejected locally rather than allowing Delhivery to silently downgrade the order to non-QC. For `multi`, the first correct value must appear in the options; image values must be HTTP(S) URLs.

Delhivery must complete the one-time client-question-ID mapping before live RVP QC use. Set those exact client IDs in `DELHIVERY_RVP_QC_QUESTION_IDS`; an empty mapping returns `503`, and an unknown ID is rejected before manifestation. Pax stores only a QC summary on the shipment and removes it from public tracking, so correct answers and doorstep inspection details are not exposed publicly. RVP QC shares the existing 18,000-request manifestation safety window below the documented 20,000 requests per five minutes.
...
NDR actions use authenticated shipment-scoped `POST /api/client/shipments/:id/ndr` and `POST /api/admin/shipments/:id/ndr` routes. Pax verifies ownership and SPS/MPS waybill membership, refreshes tracking directly from Delhivery, then sends the exact `{ "data": [{ "waybill": "...", "act": "..." }] }` contract to `/api/p/update`. `RE-ATTEMPT` is allowed only for NSL codes `EOD-74`, `EOD-15`, `EOD-104`, `EOD-43`, `EOD-86`, `EOD-11`, `EOD-69` and `EOD-6`; `PICKUP_RESCHEDULE` requires `EOD-777` or `EOD-21`, a Cancelled shipment and an explicit non-OTP cancellation. Both actions require provider attempt count 1 or 2. The customer and admin interfaces show the documented after-9-PM operational recommendation, but the server does not reject earlier requests because Delhivery describes it as guidance rather than a contract constraint.

An accepted asynchronous action is stored as an append-only Pending record with its UPL ID, and duplicate non-terminal actions for the same waybill/action are blocked. Authenticated owners and admins can refresh a stored UPL through `GET /api/client/shipments/:id/ndr/:uplId/status` or its admin equivalent. Pax calls `/api/cmu/get_bulk_upl/{uplId}?verbose=true`, normalizes the provider request state and safe per-waybill results, and persists up to 25 status checks on the matching action. A UPL must already belong to the selected shipment, preventing arbitrary request-ID probing. Unrecognized provider status values remain Pending rather than being treated as successful.

NDR eligibility metadata and action history remain available only to authenticated owners and admins; public tracking removes NSL, attempt-count, OTP-cancellation, UPL and UPL-status details. Delhivery publishes no rate limit for either NDR endpoint, so Pax uses configurable conservative 300-request/five-minute local guards. Submission uses a 130-second timeout to cover its documented 126.38-second P99; status polling uses a separate 95-second timeout above its documented 88.03-second P99.
...
The Postman/Newman suite exercises authentication, parcel and Heavy coverage, empty-list/Heavy NSZ, temporary Embargo, blocked NSZ bookings, forward/Reverse Pickup/REPL manifestation, RVP QC 3.0 two-item question mapping and privacy, QC flow/item/mapping rejection, PDF/JSON shipping labels in A4/4R, all four document-download types, document/label waybill ownership, admin-only client warehouse registration/listing/update, immutable-name enforcement, warehouse validation and duplicate protection, newly registered warehouse selection for MPS and pickup, warehouse pickup creation and listing, duplicate pickup protection, arbitrary warehouse rejection, URL-encoded SPS special characters, high-value e-waybill enforcement and updates, live SPS/MPS status and scan tracking, NDR re-attempt and pickup-reschedule eligibility, asynchronous UPL persistence and status refresh, unrelated-UPL rejection, duplicate/unsupported/ineligible NDR rejection and public NDR privacy, mandatory invoice/EWB validation, low-value EWB rejection, rejected unprefetched MPS, prepaid and COD MPS master/child fields, raw MPS JSON, shipment editing/payment conversion, terminal-status protection, Manifested/In Transit/Scheduled cancellation outcomes, duplicate cancellation protection, Surface/Express Expected TAT, Surface/Express shipping charges, optional dimensions/package type, HTTP-200 provider errors, bulk and single waybill storage, inventory lifecycle, duplicate protection and validation across customer/admin endpoints. It uses a deterministic local Delhivery contract server, so CI never needs or exposes a live provider token. Final live acceptance requires the production token, approved account details, and configured Delhivery QC question mapping; warehouse creation/update and NDR submission change live Delhivery account state and therefore are not run by CI.

Additional B2C contracts are intentionally not guessed from partial documentation. Add further adapters only after their official request/response contracts and required account identifiers are supplied.
